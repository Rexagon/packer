...
pub fn pack(base_dir: &Path, config: &Config) -> Result<(), String> {
    let config_base = config.base.as_ref().map(|base| {
        base_dir.join(Path::new(base.as_str()))
    });

    let base_dir = match config_base.as_ref() {
        Some(base) => base.as_path(),
        None => base_dir
    };

    let mut header = create_header(base_dir, config)?;

    let output = OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(true)
        .open(base_dir.join(config.output.as_str()));
    ...
    for i in 0..header.items.len() {
        println!("[{}/{}] Packing {}", i + 1, header.items.len(),
                 header.items[i].name.as_ref().unwrap());
...
impl HeaderItem {
    ...
    fn write(&mut self, pos: usize, file: &mut File) -> Result<usize, String> {
        ...
        // write offset
        file.write_u64::<LE>(self.data_offset.unwrap_or(0))
            .map_err(err_mapper)?;

        // write length
        file.write_u64::<LE>(self.data_length.unwrap_or(0))
            .map_err(err_mapper)?;
